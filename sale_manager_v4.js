...
function smBatchCopyTasks() {
  ...
  Object.keys(all).forEach(function(code){
    ...
    var reportTask = actionableTasks.find(function(t) {
      return t.code === code &&
        (t.type === 'symbol_change' || t.type === 'price_discount') &&
        t.overdueDays >= 0;
    });
    if(reportTask) {
      var over = reportTask.overdueDays;
      var base = smBasePrice(sd.symbol, pd.price || 0);
      var isPriceDiscount = reportTask.type === 'price_discount';
      var nextSym = isPriceDiscount ? sd.symbol : (reportTask.nextSym || smNextSym(sd.symbol));
      var newPrice = isPriceDiscount
        ? Math.max(0, (pd.price || 0) - SALE_DISC_AMT)
        : (nextSym ? smSymPrice(base, nextSym) : pd.price);
      var isHigh = (base >= HIGH_PRICE_ALERT);
      var itemData = {code: code, title: pd.title, sym: sd.symbol, nextSym: nextSym, oldPrice: pd.price, newPrice: newPrice, overdueDays: over, url: directUrl, actionType: reportTask.type};

      if (isHigh) {
        // 高額商品は500円値下げ・記号変更のどちらも、実行前に許可を得る。
        blockHighPrice.push(itemData);
      } else if (!isPriceDiscount && nextSym === sym80) {
        // ▲⇒〇 への変更許可願い（全商品）
        blockMaru.push(itemData);
      } else if (!isPriceDiscount && nextSym === finalSym) {
        // 〇⇒□ への変更許可願い（全商品）
  ...
  if (blockHighPrice.length > 0) {
      copyText += '■■ 【許可願い】高額商品の価格・記号変更 ■■\n';
      copyText += '（※高額商品のため、オーナーの許可をいただいてから変更します）\n';
      blockHighPrice.forEach(function(d) {
          var n = getNum();
          var highAction = d.actionType === 'price_discount'
            ? ('500円値下げ（' + (d.oldPrice||0).toLocaleString() + '円 ⇒ ' + (d.newPrice||0).toLocaleString() + '円）')
