...
function smMergeRemoteItem(localItem, remoteItem){
  ...
  var localReportStamp = localItem.ownerReportStateUpdatedAt || localItem.ownerReportDoneAt || '';
  var remoteReportStamp = remoteItem.ownerReportStateUpdatedAt || remoteItem.ownerReportDoneAt || '';
  if(localReportStamp || remoteReportStamp){
    var reportSource = remoteReportStamp >= localReportStamp ? remoteItem : localItem;
    merged.ownerReportDoneKey = reportSource.ownerReportDoneKey || '';
    merged.ownerReportDoneAt = reportSource.ownerReportDoneAt || '';
    merged.ownerReportAwaitingReplyKey = reportSource.ownerReportAwaitingReplyKey || '';
    merged.ownerReportAwaitingReplyAt = reportSource.ownerReportAwaitingReplyAt || '';
    merged.ownerInstructionReportKey = reportSource.ownerInstructionReportKey || '';
    merged.ownerReportStateUpdatedAt = reportSource.ownerReportStateUpdatedAt || reportSource.ownerReportDoneAt || '';
    if(Object.prototype.hasOwnProperty.call(reportSource, 'reportedAt')) merged.reportedAt = reportSource.reportedAt;
  }
  return merged;
}
...
// 現在オーナー報告が必要な内容を、商品ごとに一意のキーへ変換する。
// 同じキーだけを報告済みとして抑止し、次の記号・期限は別報告として扱う。
function smGetOwnerReportCandidate(code, sd, item, allTasks){
  if(!item || !smIsReportEligible(item)) return null;
  ...
  if(sd.symbol === finalSym){
    ...
    if(passedDays >= reportDays){
      return {
        key:'FINAL|'+code+'|'+baseDate+'|'+sd.symbol,
        kind:'final', dueDate:smAddDays(baseDate, reportDays), overdueDays:passedDays
      };
    }
    return null;
  }
  var tasks = allTasks || smGetAllTasks();
  var task = tasks.find(function(t){
    return t.code===code && t.type==='symbol_change' && t.overdueDays>=0;
  });
  if(!task) return null;
  var nextSym = task.nextSym || smNextSym(sd.symbol);
  return {
    key:'SYMBOL|'+code+'|'+task.taskId+'|'+task.dueDate+'|'+sd.symbol+'|'+nextSym,
    kind:'symbol', dueDate:task.dueDate, overdueDays:task.overdueDays,
    task:task, nextSym:nextSym
  };
}

function smGetPendingOwnerReport(){
  try { return JSON.parse(localStorage.getItem('sm_owner_report_copy_pending_v1') || 'null'); }
  catch(e){ return null; }
}

function smMarkCopiedOwnerReportSent(){
  var pending = smGetPendingOwnerReport();
  if(!pending || !pending.items || !pending.items.length){
    alert('送信済みにする報告がありません。先に「本日の報告をコピー」を押してください。');
    return;
  }
  if(!confirm('コピーした本日の報告を、実際にオーナーへ送信しましたか？\n\n「OK」で同じ内容を報告済みにします。')) return;
  var stateUpdatedAt = new Date().toISOString();
  pending.items.forEach(function(entry){
    var sd = smGetItem(entry.code);
    sd.ownerReportDoneKey = entry.key;
    sd.ownerReportDoneAt = stateUpdatedAt;
    sd.ownerReportAwaitingReplyKey = entry.key;
    sd.ownerReportAwaitingReplyAt = stateUpdatedAt;
    sd.ownerReportStateUpdatedAt = stateUpdatedAt;
    if(entry.kind === 'final') sd.reportedAt = smTodayStr();
    smSetItem(entry.code, sd);
  });
  localStorage.removeItem('sm_owner_report_copy_pending_v1');
  smRenderAll();
  showToast('✅ オーナーへの送信済みとして保存しました', 2200);
}
...
function smRenderPanel(item){
  ...
  var awaitingReplyKey = sd.ownerReportAwaitingReplyKey || '';
  var needsInstructionReply = !!awaitingReplyKey && sd.ownerInstructionReportKey !== awaitingReplyKey;
  var instructionReminderHtml = needsInstructionReply
...
function smSaveOwnerInstruction(code){
  ...
  if(sd.ownerInstruction){
    var reportKey = sd.ownerReportAwaitingReplyKey || '';
    var currentCandidate = null;
    if(!reportKey){
      var item = items.find(function(i){ return i.code===code; });
      currentCandidate = item ? smGetOwnerReportCandidate(code, sd, item) : null;
      reportKey = currentCandidate ? currentCandidate.key : '';
    }
    if(reportKey){
      var stateUpdatedAt = new Date().toISOString();
      sd.ownerInstructionReportKey = reportKey;
      sd.ownerReportDoneKey = reportKey;
      sd.ownerReportDoneAt = stateUpdatedAt;
      sd.ownerReportAwaitingReplyKey = '';
      sd.ownerReportAwaitingReplyAt = '';
      sd.ownerReportStateUpdatedAt = stateUpdatedAt;
      if(currentCandidate && currentCandidate.kind === 'final') sd.reportedAt = smTodayStr();
    }
  }
  smSetItem(code, sd);
  _smSelected = code;
...
function smOnLikes(code){
  ...
  if(nextTask && nextTask.type === 'price_discount'){
    ...
    var yahooDiscountHtml = (typeof CONFIG !== 'undefined' && CONFIG.YAHOO_AUCTION_STYLE_ENABLED === true)
      ? '<span style="font-size:0.85em;color:#94a3b8;">開始:</span> <b style="color:#fdba74;font-size:1.15em;">¥'+targetPrice.toLocaleString()+'</b>'+cbtn(targetPrice)+'&nbsp;&nbsp;<span style="font-size:0.85em;color:#94a3b8;">即決:</span> <b style="color:#fdba74;font-size:1.15em;">¥'+yaSokketu.toLocaleString()+'</b>'+cbtn(yaSokketu)
  ...
  if(!nextSym){
    ...
    if (d >= reportDays) {
        html += '<div style="margin-top:12px;border:1px solid rgba(251,191,36,.35);color:#fbbf24;border-radius:7px;padding:9px;font-size:.8rem;text-align:center;font-weight:600;">上部の「本日の報告をコピー」から送信してください</div>';
    }
    result.innerHTML = html;
    return;
  }
  ...
  if(hasSale){
    ...
    var yahooSaleHtml = (typeof CONFIG !== 'undefined' && CONFIG.YAHOO_AUCTION_STYLE_ENABLED === true)
      ? '<span style="font-size:0.78rem;color:#64748b;">最低入札</span>&nbsp;<b style="color:#94a3b8;font-size:1.05em;">¥'+nextPrice.toLocaleString()+'</b>'+cbtn(nextPrice)+'&nbsp;&nbsp;<span style="font-size:0.78rem;color:#64748b;">即決</span>&nbsp;<b style="color:#94a3b8;font-size:1.05em;">¥'+_yaSokketu.toLocaleString()+'</b>'+cbtn(_yaSokketu)
      : '<span style="font-size:0.78rem;color:#64748b;">定額</span>&nbsp;<b style="color:#94a3b8;font-size:1.05em;">¥'+nextPrice.toLocaleString()+'</b>'+cbtn(nextPrice);
  ...
  var yahooChangeHtml = (typeof CONFIG !== 'undefined' && CONFIG.YAHOO_AUCTION_STYLE_ENABLED === true)
    ? '<span style="font-size:0.85em;color:#94a3b8;">開始:</span> <b style="color:#fdba74;font-size:1.15em;">¥'+nextPrice.toLocaleString()+'</b>'+cbtn(nextPrice)+'&nbsp;&nbsp;<span style="font-size:0.85em;color:#94a3b8;">即決:</span> <b style="color:#fdba74;font-size:1.15em;">¥'+yaSokketu.toLocaleString()+'</b>'+cbtn(yaSokketu)
    : '<span style="font-size:0.85em;color:#94a3b8;">定額:</span> <b style="color:#fdba74;font-size:1.15em;">¥'+nextPrice.toLocaleString()+'</b>'+cbtn(nextPrice);
...
document.addEventListener('click', function(e) {
  ...
  if (e.target && e.target.id === 'btn-mark-owner-report-sent') {
    smMarkCopiedOwnerReportSent();
  }
});
...
function smBatchCopyTasks() {
  ...
  Object.keys(all).forEach(function(code){
    ...
    var candidate = smGetOwnerReportCandidate(code, sd, pd, actionableTasks);
    if(!candidate) return;

    // 旧版で既に返信内容を保存済みなら、期限日以降の保存を同じ報告への返信として移行する。
    if(!sd.ownerReportDoneKey && sd.ownerInstruction && sd.ownerInstructionAt && candidate.dueDate && sd.ownerInstructionAt >= candidate.dueDate){
      sd.ownerReportDoneKey = candidate.key;
      sd.ownerInstructionReportKey = candidate.key;
      sd.ownerReportDoneAt = sd.ownerInstructionUpdatedAt || sd.ownerInstructionAt;
      sd.ownerReportStateUpdatedAt = sd.ownerInstructionUpdatedAt || (sd.ownerInstructionAt + 'T00:00:00.000Z');
      if(candidate.kind === 'final') sd.reportedAt = sd.ownerInstructionAt;
      smSetItem(code, sd);
    }
    if(sd.ownerReportDoneKey === candidate.key) return;
  ...
  navigator.clipboard.writeText(copyText).then(function() {
    var copiedItems = blockOverdue.concat(blockHighPrice, blockMaru, blockHighSale, blockShikaku).map(function(d){
      return {code:d.code, key:d.reportKey, kind:d.reportKind};
    });
