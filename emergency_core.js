...
(function(root){
 ...
 function record(job,checks,at){
  ...
  assert(checks&&Object.keys(job.platforms).every(p=>{
   ...
   return c.status==='done'&&c.confirmed===true&&job.platforms[p]!==null&&Number.isSafeInteger(c.before)&&c.before>=job.platforms[p]&&c.price===job.platforms[p]&&c.symbolConfirmed===true;
