...
function entries(file){
 ...
 for(const e of file.activity||[]){
  ...
  }else if(e.status!=='unlisted'||e.platform==='shops'||e.priceChanged!==false||e.symbolChanged!==false)throw Error('対象外記録が不正です');
  result.push(e);
 }
...
function summarize(files){
 ...
 for(const e of rows){const day=new Date(Date.parse(e.completedAt)+9*3600000).toISOString().slice(0,10),key=day+' '+e.role;
  const g=groups[key]||(groups[key]={day,role:e.role,price:0,symbol:0,both:0,changes:0,checkOnly:0,unlisted:0,unknown:0});
