...
function event(batch,role,job,platform,c,at){
 ...
 if(!c||!(c.status==='auction'&&['mercari','yahoo_auction'].includes(platform)||c.status==='missing'&&platform!=='shops'||c.status==='sold'&&c.reported===true)&&(!c.confirmed||!['done','unlisted'].includes(c.status)||platform==='shops'&&c.status==='unlisted'))throw Error('この販路の商品と作業結果を確認してください');
...
function entries(file){
 ...
 for(const e of file.activity||[]){
  ...
  }else if(!['unlisted','missing','sold','auction'].includes(e.status)||(e.status==='auction'&&!['mercari','yahoo_auction'].includes(e.platform))||(e.platform==='shops'&&e.status!=='sold')||e.priceChanged!==false||e.symbolChanged!==false)throw Error('対象外記録が不正です');
  result.push(e);
 }
 ...
 for(const r of file.records||[])for(const [platform,c] of Object.entries(r.checks||{})){
  ...
  if(c.status==='auction'&&!['mercari','yahoo_auction'].includes(platform))throw Error('オークション対象販路が不正です');
...
function summarize(files){
 ...
 for(const e of rows){const day=new Date(Date.parse(e.completedAt)+9*3600000).toISOString().slice(0,10),key=day+' '+e.role;
  const g=groups[key]||(groups[key]={day,role:e.role,price:0,symbol:0,both:0,changes:0,checkOnly:0,unlisted:0,missing:0,sold:0,auction:0,unknown:0});
