...
function smBatchCopyTasks() {
  ...
  Object.keys(all).forEach(function(code){
    ...
    // 2. 許可願い：タスクが期限超過（overdueDays >= 0）している記号変更
    if(sd.tasks) {
      sd.tasks.forEach(function(t) {
        if(t.status === 'done') return;
        var over = smDaysDiff(t.dueDate);
        if(over < 0) return; // まだ期限前はスキップ

        var base = smBasePrice(sd.symbol, pd.price || 0);
        var nextSym = t.nextSym || smNextSym(sd.symbol);
        ...
        if (isHigh) {
          // 高額商品：記号変更はすべて許可願い
          if(t.type === 'symbol_change') blockHighPrice.push(itemData);
