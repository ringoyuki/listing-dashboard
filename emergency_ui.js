(async function(){
 ...
 function copyButton(label,value,parent){
  ...
  const success=()=>{parent.querySelectorAll('.copy-selected').forEach(el=>el.classList.remove('copy-selected'));b.classList.add('copy-selected');feedback.textContent='コピーしました：'+value;b.textContent='✓ コピー済み：'+value;};
 ...
 function staff(parent){node('h2','スタッフ：A用／B用ファイルを読み込む',parent);
  ...
  w.jobs.forEach((j,index)=>{const section=node('details',undefined,parent);section.dataset.productCode=j.code;cards.push({section,j});section.className='product-card '+(index%2?'product-alternate':'')+(state.records[j.code]?' product-complete':'');node('summary',(state.records[j.code]?'完了：':'未完了：')+j.code+' ／ '+j.baseItem.title,section);node('h2','変更後の記号：'+j.symbol+' ／ Shops指定価格 '+j.price.toLocaleString()+'円',section);
   ...
   Object.keys(j.platforms).forEach(p=>{const box=node('section',undefined,section),target=j.platforms[p];node('small','管理番号：'+j.code,box);node('h3',labels[p]+'：'+(target===null?'オーナー確認':target.toLocaleString()+'円に合わせる'),box);node('h3','変更後の記号： '+j.symbol,box);
    ...
    if(p==='shops'&&j.shopsOwnerManaged&&d.status!=='owner_csv'){box.replaceChildren();node('h3','メルカリShops：オーナーが一括対応',box);node('p','スタッフの変更操作は不要です。他の販路を進めてください。オーナーの反映確認後、この画面へ変更済み情報が反映されます。',box);return;}
    box.classList.add('platform-card','platform-'+p);
    if(d.status==='owner_csv'){box.classList.add('platform-finished');node('strong','✓ オーナーがCSVで変更済み',box);node('p','Shopsの作業は不要です。他の販路を進めてください。スタッフの変更件数には加算しません。',box);return;}
    ...
    if(['mercari','yahoo_auction'].includes(p)){
     button('オークション中のためスキップ',async()=>{status.value='auction';await status.onchange();},box);
     node('small','入札者が0人でもスキップします。オークション中かどうかは商品ページで確認してください。',box);
    }
    showPlatformState();
   ...
   const hold=button('要確認として保存して次へ',async()=>{
    ...
    await save();section.open=false;section.classList.add('product-held');
    const cards=Array.from(app.querySelectorAll('[data-product-code]'));const next=cards[cards.indexOf(section)+1];
 ...
 function render(){
  ...
  ownerSwitch(app);
  const isOwnerView=new URLSearchParams(location.search).get('ownerSwitch')==='1';
  const staffBox=node(isOwnerView?'details':'section',undefined,app);
  if(isOwnerView){
   node('summary',w?w.role+'さんの作業一覧：'+total+'件中 '+completed+'件完了 ／ 残り '+(total-completed)+'件（クリックで開閉）':'A・B担当ファイルと作業一覧（クリックで開閉）',staffBox);
   staffBox.open=ownerWorkExpanded;
